...
set_time_limit(0);
	...
	if(!empty($_GET['folder']))
	{
		...
		else
		{
			...
			{
				$result1 = mysql_query("SELECT * FROM fr_activityfolder  WHERE folder_name = '".$url_folder."'");
				if(mysql_num_rows($result1) > 0)
				{
					$row1 = mysql_fetch_array($result1);
					
					  $dir_to_browse2 .= $row1['folder_path']."/";
				}
				else
				{
...
<script type="text/javascript">
	...
	function mouse_out_handler(event)
	{
		...
		while (toElement && toElement.id != "file_edit_box")
							...
							<div id="toolbar-1011" class="x-toolbar x-unselectable x-tabpanel-child x-toolbar-default x-box-layout-ct" style="border-width: 0pt; margin: 0pt; width: 100%;">
								<div id="toolbar-1011-innerCt" class="x-box-inner" style="width: 100%; height: 95px;">
									...
									if(mysql_num_rows($result) > 0)
									{
											...
											$result = mysql_query("SELECT * FROM fr_share_folder WHERE Folder_Name  = '".$url_folder."' AND studID = '".$_SESSION['uid']."'") or die ("folder error");
											if(mysql_num_rows($result) > 0)
											{
												$row = mysql_fetch_array($result);
											?>

												<div id="buttongroup-1037" class="x-btn-group x-box-item x-toolbar-item x-btn-group-default" style="margin: 0pt; right: auto; left: 71px; top: 0px; height: 90px; width: 133px;">
...
<div id="file_edit_box" onmouseout="mouse_out_handler(event);">
  <table width="100%" border="0" class="table_border" >
    ...
    <tr>
    	 ...
    	 <ul>
    		<li>
    			<img src="Images/folderopened.png" width="20px" height="20px">
    		</li>
	    	<li>
	    		Set Deadline
	    	</li>
...
function share_folder()
{
	...
	//fg_popup_form("fg_formshare","fg_form_InnerContainer","fg_backgroundpopup");
...
function set_deadline()
{
	...
	window.open('FileManagement/activityfolder.php?folder=<?PHP echo base64_decode($_GET['folder']); ?>&item_name='+item_name_base64, null, 'scrollbars = 0, status = 1, height = 500, width = 470, resizable = 1, location = 0');


	//fg_popup_form("fg_formActivity","fg_form_InnerContainer","fg_backgroundpopup");
...
//require('FileManagement/share_form.php');
//require('FileManagement/activityfolder.php');

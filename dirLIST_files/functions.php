							...
							else
							{
								...
								while ($row = mysql_fetch_array($result)) 
								{
									...
									if(mysql_num_rows($result3) > 0)
									{
										...
										{
											$result4 = mysql_query("SELECT fr_share_folder.*,fr_deadline.* FROM  fr_share_folder,fr_deadline  WHERE fr_share_folder.subID = fr_deadline.folder_id ");
											if(mysql_num_rows($result4) > 0)
											{
												$row4 = mysql_fetch_array($result4);

												$diff = $row4['time_deadline'] - date("h:i");
												if($row4['date_deadline'] == date('Y-m-d') && $diff > 0)
												{

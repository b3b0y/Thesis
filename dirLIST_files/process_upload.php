	...
	if($_POST['submit'] == $local_text['upload'])
	{
		...
		if(!empty($folder))
		{
			...
			else
			{
				...
				{
					$result3 = mysql_query("SELECT * FROM fr_activityfolder  WHERE folder_name = '".$folder."'");
					if(mysql_num_rows($result3) > 0)
					{
						$row3 = mysql_fetch_array($result3);
						
						$diff = $row3['Time_deadline'] - date("h:i");
						if($row3['Date_deadline'] == date('Y-m-d') && $diff > 0)
						{
						   $dir_to_browse = $row3['folder_path']."/";
							(substr($folder, -1, 1) != "/" && !empty($folder) ? $folder .= "/" : $folder);
							$new_path = $dir_to_browse.$file_name;
						...
						{
							echo "<script> alert('your late'); window.location='../index.php?folder=".$_POST['folder']."'; </script>";
							exit;
						}
					}
					else

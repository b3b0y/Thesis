	...
	{ 
		...
		if($view_mode == 0) //thumbnail mode
		{
			...
			foreach($folders_sorted as $key => $val)//This part is for the folders
			{
				...
				$cell_name .= '<a  id="clickf'.$folders_counter.'"  href="'.$this_file_name.'?folder='.base64_encode($folders['link'][$key]).'"><img border="0" style="cursor:pointer">'.chunk_split($folders['name'][$key], 15, "<br />").'</a>';

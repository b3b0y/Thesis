       	...
       	<ul>
			<li>
	       		<a href="#">
		       		...
		       		<?php 
		       			$result1 = mysql_query("SELECT DISTINCT folder_name,folder_owner,url FROM notification WHERE status = 0 AND folder_owner = '".$_SESSION['uid']."'");
		       			...
		       			$enroll_count=mysql_num_rows($result);
		       			
		       			$count += $enroll_count;
		       			if($count!=0)
						{
		       		?>
		       		<span id="mes"><?php echo $count ?></span>
		       			<?php
		       			}
		       			?>
	       		</a>
	       		<ul class="sub-menu1">
					...
					?>	
		       			...
		       			<div class="bbbbbbb" >
							...
							<a href='Admin.php?approval=Subject' class="view_comments" id="<?php echo $id; ?>">View all <?php echo $enroll_count; ?> enrolled student</a></div>
						</div>
		       			</li>
	       			<?php
					}

					$result = mysql_query("SELECT DISTINCT folder_name,folder_owner,url FROM notification WHERE status = 0");
					...
					?>	
				       			...
				       			<div class="bbbbbbb" >
									<div style="background-color: #F7F7F7; border-bottom-left-radius: 3px; border-bottom-right-radius: 3px; position: relative; z-index: 100; padding:8px; cursor:pointer;">
									<a href='<?php echo $row["url"]; ?>' class="view_comments" id="<?php echo $id; ?>">View all <?php echo $row['folder_name']; ?> folder</a></div>
								</div>
				       			</li>
	       			<?php
